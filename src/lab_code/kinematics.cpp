...
TaskSpace forwardKinematics(JointSpace state) {
    ...
    // You may need the variables: L1, L2, state.q1, state.q2
...
JointSpace inverseKinematics(TaskSpace point) {
    ...
    // Makes sure that q1 and q2 are in [-M_PI/2, M_PI/2]
    state.q1 = atan2(sin(state.q1), cos(state.q1));
    state.q2 = atan2(sin(state.q2), cos(state.q2));
