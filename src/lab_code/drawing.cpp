...
JointSpace state;
JointSpace setpoint = {Q1_OFFSET, 0.0};
...
bool safetyLimit(JointSpace state) {
    return abs(state.q1 - M_PI/2.0) <= M_PI/2.0 &&
           abs(state.q2) <= M_PI*0.9;
};
...
void loop() {
    ...
    EVERY_N_MICROS(500) {
        ...
        endEffectorState = {q1, q2};
        endEffectorActual = forwardKinematics(endEffectorState);
    }

    // Update PID at 10kHz
    EVERY_N_MICROS(100) {        
        q1 = encoder1.getPosition() + Q1_OFFSET;
        q2 = -encoder2.getPosition();
        controlEffort1 = motorPID1.calculateParallel(q1, setpoint.q1);
        controlEffort2 = motorPID2.calculateParallel(q2, setpoint.q2);
